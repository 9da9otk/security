...
let routeMode = false;
let routes = []; // مصفوفة لتخزين المسارات المتعددة
let currentRouteIndex = -1; // مؤشر المسار النشط
let routePoints = [];
...
function createStopMarker(position, index, routeIndex) {
  const m = new google.maps.Marker({
    ...
    label: { text: String(index + 1), color: routes[routeIndex].style.color, fontSize: '11px', fontWeight: '700' },
    clickable: true,
    draggable: !shareMode
  });
  m.addListener('dragend', () => {
    routes[routeIndex].points[index] = m.getPosition();
...
function clearRouteVisuals(routeIndex) {
  if (routes[routeIndex].stopMarkers) {
    routes[routeIndex].stopMarkers.forEach(m => m.setMap(null));
...
// --- MAIN BOOT FUNCTION ---
function boot() {
  console.log('Booting Diriyah Map v18.0');
