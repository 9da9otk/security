/* Diriyah Security Map – v12.2 (✅ fixed: route modifications now appear in shared links) */
'use strict';
/* ---------------- Robust init ---------------- */
...
let btnTraffic, btnShare, btnAdd;
let modeBadge, toast;
let mapTypeSelector;
/* Route globals */
...
function compressState(state) {
  ...
  // 🔧 إصلاح: تضمين بيانات المسار بشكل صحيح
  if(state.r && (state.r.ov || state.r.points)) {
    compressed.r = {
      ov: state.r.ov || '',
      points: state.r.points || [],
      style: state.r.style || routeStyle // 🔧 إضافة إعدادات النمط
    };
  }
...
function requestAndRenderRoute(){
  ...
  directionsService.route(req, (result, status) => {
    if(status === 'OK' && result){
      ...
      persist(); // 🔧 إضافة: حفظ التغييرات تلقائياً
    } else {
...
function extractActivePolyline(){
  ...
  persist(); // 🔧 إضافة: حفظ المسار المستخرج
}

// 🔧 إصلاح: دالة محسنة لاستعادة المسار
function restoreRouteFromOverview(polyStr, routePointsArray = null, routeStyleData = null){
  ...
  try{
    ...
    // 🔧 تطبيق إعدادات النمط إذا كانت موجودة
    if(routeStyleData){
    ...
    // 🔧 إعادة إنشاء علامات النقاط إذا كانت البيانات متوفرة
    if(routePoints.length > 0){
      routeStopMarkers = routePoints.map((pos, i) => createStopMarker(pos, i));
    }
    
    // 🔧 تحديث العارض إذا كان موجوداً
    if(directionsRenderer){
...
function attachRouteCardEvents(){
  ...
  function apply(){
    ...
    persist(); // 🔧 إضافة: حفظ التغييرات فوراً
  }
...
function applyState(s){
  ...
  // 🔧 إصلاح: استعادة المسار مع جميع البيانات
  if(s.r){
...
function boot(){
  ...
  // ... باقي كود التهيئة بدون تغيير ...
  // (يبقى نفس كود التهيئة السابق مع التأكد من استدعاء persist في الأحداث المهمة)
}

// ... باقي الدوال تبقى كما هي مع التأكد من استدعاء persist عند التعديلات ...
...
function buildState(){
  ...
  // 🔧 إصلاح: تضمين جميع بيانات المسار بشكل كامل
  const r = (currentRouteOverview || routePoints.length > 0) ? {
    ov: currentRouteOverview || '',
    points: routePoints.map(p => ({ lat: p.lat(), lng: p.lng() })),
    style: { ...routeStyle } // 🔧 تضمين إعدادات النمط
  } : null;
  ...
  return {
    ...
    c: cRows,
    n: nRows,
    r // 🔧 التأكد من تضمين بيانات المسار
  };
}
